...
from .seq2seq import InstructionEncoder
...
class CMA(nn.Module):
    ...
    def __init__(
        ...
        # input encoders
        self.action_encoder = InstructionEncoder(final_state_only=False, bidirectional=True,vocab_size=30000)
        ...
        self.first_state_encoder = build_rnn_state_encoder(
            input_size=rgb_encoder_output_size * 5,
        ...
        self.rgb_kv = nn.Conv1d(self.rgb_encoder.output_shape[0] * 5, hidden_size // 2 + rgb_encoder_output_size, 1)
        ...
        self.second_state_compress = nn.Sequential(
            nn.Linear(
                (
                    hidden_size
                    + rgb_encoder_output_size * 2
                    + self.instruction_encoder.output_size * 2
                ), 
                hidden_size
            ),
            nn.ReLU(True),
        ...
        # prediction heads
        self.action_head = self.prediction_head = nn.Sequential(
    ...
    def forward(
        ...
        cur_rgb_embedding = self.rgb_encoder(cur_rgb_batch).flatten(2)
        his_rgb_embedding_4 = self.rgb_encoder(his_rgb_batch_4).flatten(2)
        his_rgb_embedding_3 = self.rgb_encoder(his_rgb_batch_3).flatten(2)
        his_rgb_embedding_2 = self.rgb_encoder(his_rgb_batch_2).flatten(2)
        his_rgb_embedding_1 = self.rgb_encoder(his_rgb_batch_1).flatten(2)

        cur_rgb_features = self.rgb_linear(cur_rgb_embedding)
        his_rgb_features_4 = self.rgb_linear(his_rgb_embedding_4)
        his_rgb_features_3 = self.rgb_linear(his_rgb_embedding_3)
        his_rgb_features_2 = self.rgb_linear(his_rgb_embedding_2)
        his_rgb_features_1 = self.rgb_linear(his_rgb_embedding_1)

        # 1st GRU
        state_in = torch.cat([cur_rgb_features, his_rgb_features_4, his_rgb_features_3, his_rgb_features_2,his_rgb_features_1], dim=1)
        ...
        instruction_k = self.text_k(instruction_embedding)
        instruction_mask = (instruction_embedding == 0.0).all(dim=1)
        attended_instruction = self._attn(x_q, instruction_k, instruction_embedding, instruction_mask)

        action_k = self.text_k(action_embedding)
        action_mask = (action_embedding == 0.0).all(dim=1)
        attended_action = self._attn(x_q, action_k, action_embedding, action_mask)

        # attended text <-> rgbd cross attention

        all_rgb_embedding = torch.cat(
            [cur_rgb_embedding, his_rgb_embedding_4, his_rgb_embedding_3, his_rgb_embedding_2, his_rgb_embedding_1], dim=1
        )
        rgb_k, rgb_v = torch.split(self.rgb_kv(all_rgb_embedding), self._hidden_size // 2, dim=1)

        instruction_q = self.text_q(attended_instruction)
        attended_instruction_rgb = self._attn(instruction_q, rgb_k, rgb_v)

        action_q = self.text_q(attended_action)
        attended_action_rgb = self._attn(action_q, rgb_k, rgb_v)



        # 2nd GRU
        x = torch.cat([x, attended_instruction, attended_action, attended_instruction_rgb, attended_action_rgb], dim=1)
