        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # Reuse the per-token log probabilities returned by vLLM when requested.
                    # They are generated by the rollout policy before the single optimizer
                    # update of this global step, so they are the detached old-policy values.
                    with marked_timer("old_log_prob", timing_raw, color="blue"):
                        reuse_rollout_log_probs = self.config.algorithm.get(
                            "reuse_rollout_log_probs_as_old", False
                        )
                        if reuse_rollout_log_probs:
                            if "rollout_log_probs" not in batch.batch:
                                raise RuntimeError(
                                    "reuse_rollout_log_probs_as_old requires "
                                    "actor_rollout_ref.rollout.calculate_log_probs=true"
                                )
                            batch.batch["old_log_probs"] = batch.batch["rollout_log_probs"].detach()
                            batch.meta_info["temperature"] = self.config.actor_rollout_ref.rollout.temperature
                            metrics["training/reused_vllm_old_log_probs"] = 1.0
                        else:
                ...
                if should_save:
                    ...
                    # The actor checkpoint manager rotates actor subdirectories. Remove
                    # stale top-level global_step directories (for example their data.pt)
                    # as well, so max_actor_ckpt_to_keep=1 truly retains one checkpoint.
                    if self.config.trainer.get("max_actor_ckpt_to_keep", None) == 1:
                        current_name = f"global_step_{self.global_steps}"
                        ...
                        for entry in os.scandir(checkpoint_root):
                            if entry.is_dir() and entry.name.startswith("global_step_") and entry.name != current_name:
