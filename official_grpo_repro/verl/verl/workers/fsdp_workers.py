...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        # Reference parameters are frozen. Respect the configured placement:
        # resident GPU parameters avoid a CPU<->GPU transfer on every ref pass,
        # while param_offload=true retains the memory-saving behavior.
        cpu_offload = CPUOffload(offload_params=True) if (
            role != "actor" and fsdp_config.get("param_offload", False)
        ) else None
        ...
        if fsdp_strategy == "fsdp":
            ...
            if role == "actor" and fsdp_config.offload_policy:
                ...
                cpu_offload = CPUOffloadPolicy(pin_memory=True) if (
                    role != "actor" and fsdp_config.get("param_offload", False)
                ) else None
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_config.mode == "sync" and self._is_actor:
            try:
                ...
                # Python 3.10+ no longer creates a main-thread event loop on
                # demand under uvloop.  Sync rollout still needs one here to
                # enter trainer mode during worker initialization.
